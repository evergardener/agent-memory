...
Agent Memory 是本地优先、证据驱动的 Hermes 长期记忆系统。当前不可变测试版本仍是 `1.0.0-rc.6`；其后的阶段 C 实现已提交并通过当前小样本验收，但尚未打包为新候选版本。系统支持 Hermes 多 profile 共享身份、只读原始证据、事实/情节/长期脉络、三路召回、生命周期治理、确定性互动状态、整理报告、关系型星系和独立加密 Vault。
