...
Agent Memory 是本地优先、证据驱动的 Hermes 长期记忆系统。当前源码候选为 `1.0.0-rc.7`，已通过隔离上线前 Review，但尚未打 tag、部署正式 namespace 或接入生产 Hermes。系统支持 Hermes 多 profile 共享身份、只读原始证据、事实/情节/长期脉络、三路召回、生命周期治理、确定性互动状态、整理报告、关系型星系和独立加密 Vault。
...
- 全新生产预部署与单 profile canary：[`docs/V1.0-生产预部署接入试运行手册.md`](docs/V1.0-生产预部署接入试运行手册.md)
...
全量候选版本检查会使用独立 project/data/network/image 前缀构建镜像，运行 API/Hermes 集成、worker 故障恢复以及备份恢复演练：
...
发布 Gate 通过后，生产预部署必须创建全新容器和运行资产，不能复用当前测试栈：
...
bash scripts/predeploy-backup.sh "$runtime_root/predeploy.env"
```

此时系统仍为空、模型关闭且没有 Hermes profile 接入。如何准备和验证单 profile canary 见生产预部署手册。
