...
`1.0.0-rc.7` 源码候选已完成上线前 Review 和隔离 Gate，阶段 C 尚未进入正式 namespace、生产 Hermes 或正式 tag/release。
...
| P0 | 生成提交后 rc.7 正式 Gate 与产物 | 干净 HEAD、OCI revision、全量 Gate、源码包与 SHA 一致；不重建在线栈 |
| P0 | 生产备份副本预演 | `0013 -> 0014`、evidence 哈希、恢复、Vault 和旧 API 兼容性通过 |
| P0 | 获取三项上线授权 | 正式 namespace、rc.7 发布、单 profile 灰度分别确认 |
...
并知道正式迁移仍需恢复 `.env`、生产备份副本预演和三项用户授权。达到这些条件即可继续，无需重新分析需求。
