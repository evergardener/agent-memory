...
旧临时 `7810` 预部署已于 2026-07-21 无损停止，未接入 Hermes。下一个运行栈将直接使用最终生产 project/data/Vault/namespace 做真实单 profile canary，通过后原地晋级。
...
| 优先级 | 任务 | 完成标准 |
| --- | --- | --- |
| P0 | 创建最终生产候选资产 | 固定 `agent-memory-production`/data/Vault/`hermes:user-primary`，空数据 Gate 和备份恢复通过 |
