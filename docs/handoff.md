...
V1 计划内核心能力和阶段 C 关系星系已完成；`1.0.0-rc.7` 源码候选已通过上线前 Review 和隔离 Gate。
当前所有运行容器均为测试资产；生产预部署必须创建全新容器，并从独立 Hermes/单 profile canary 开始。
...
8. [`V1.0-生产预部署接入试运行手册.md`](V1.0-生产预部署接入试运行手册.md)：全新容器与 canary 操作顺序。
...
根目录 `.env` 当前不存在，但这只影响管理现有测试栈。正式预部署不恢复或复用它，而是由
`init-predeploy-env.sh` 在仓库外生成全新 `predeploy.env`、data、Vault key 和备份目录。现有测试
`secrets/vault_root_key` 禁止复制到新预部署。
...
| 优先级 | 任务 | 完成标准 |
| --- | --- | --- |
| P0 | 创建全新生产预部署 | 独立 project/data/Vault/namespace，空数据 Gate、备份恢复和状态清单通过 |
| P0 | 准备独立 Hermes canary | 用户指定非关键 profile 后生成 `0600` canary env，不修改现有 Hermes |
| P0 | 获取接入授权 | 独立 Hermes 试连和单 profile 灰度分别确认 |
...
并知道正式预部署必须新建容器，现有测试 `.env` 和数据库不是上线前置或生产源。空栈与备份 Gate 通过后即可申请独立 Hermes/单 profile canary 授权。
